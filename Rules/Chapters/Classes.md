...
Some abilities don't cost your Heroic Resource to use but allow you to spend your Heroic Resource to enhance or add effects to the ability, such as the [conduit](REL_PATH_PREFIXRules/Classes/ConduitREL_PATH_SUFFIX)'s Healing Grace. These abilities are not heroic abilities unless the baseline ability can't be used without spending your Heroic Resource.
...
When an ability can target creatures and objects, the ability can damage objects. However, unless otherwise noted (as with the [talent](REL_PATH_PREFIXRules/Classes/TalentREL_PATH_SUFFIX)'s Minor Telekinesis ability) or if the Director allows it, objects are immune to an ability's other effects. If an ability forces an object to make a test, the object automatically gets a tier 1 result on the test.
...
Using the [fury](REL_PATH_PREFIXRules/Classes/FuryREL_PATH_SUFFIX)'s Brutal Slam ability as an example again, that ability uses a Might power roll and features the following damage expressions in the three tier outcomes of the power roll:
