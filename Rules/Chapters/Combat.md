...
The minimum amount of space a creature can take up during combat is 1 square, but size 1 creatures can run the range from tiny pixies to small [polders](REL_PATH_PREFIXRules/Ancestries/PolderREL_PATH_SUFFIX), medium [humans](REL_PATH_PREFIXRules/Ancestries/HumanREL_PATH_SUFFIX), and large [hakaan](REL_PATH_PREFIXRules/Ancestries/HakaanREL_PATH_SUFFIX). As such, for creatures of size 1, that size is further broken down as 1T, 1S, 1M, or 1L—abbreviations for tiny, small, medium, and large. Size 1T is one size smaller than size 1S, two sizes smaller than 1M, three sizes smaller than 1L, and four sizes smaller than size 2. If a mechanic mentions size 1 creatures, that mechanic applies to all creatures of size 1.
...
| ---- | --------------------------------------------------------------- |
| 1T   | Pixie                                                           |
| 1S   | [Polder](REL_PATH_PREFIXRules/Ancestries/PolderREL_PATH_SUFFIX) |
| 1M   | [Human](REL_PATH_PREFIXRules/Ancestries/HumanREL_PATH_SUFFIX)   |
| 1L   | [Hakaan](REL_PATH_PREFIXRules/Ancestries/HakaanREL_PATH_SUFFIX) |
| 2    | Ogre                                                            |
| 3    | Shambling mound                                                 |
| 4    | Hill giant                                                      |
| 5    | Omen dragon                                                     |
...
It takes extra effort to dig vertically through the ground as opposed to tunneling horizontally, requiring a creature to use a special maneuver. To use the [Dig maneuver](REL_PATH_PREFIXRules/Movement/Dig%20ManeuverREL_PATH_SUFFIX), a creature must have "burrow" in their speed entry, they must have a speed that is equal to or greater than their size, and they must be touching terrain that can be burrowed through.

When a creature uses the [Dig maneuver](REL_PATH_PREFIXRules/Movement/Dig%20ManeuverREL_PATH_SUFFIX), they can move vertically up to a number of squares equal to their size. If a [burrowing](REL_PATH_PREFIXRules/Movement/BurrowREL_PATH_SUFFIX) creature has a creature who is not unconscious [grabbed](REL_PATH_PREFIXRules/Conditions/GrabbedREL_PATH_SUFFIX), they can't willingly move deeper into the ground. It's too difficult to dig with a flailing enemy in your claws.
...
While a creature who is completely beneath the ground while [burrowing](REL_PATH_PREFIXRules/Movement/BurrowREL_PATH_SUFFIX) is force moved by movement that isn't [vertical](REL_PATH_PREFIXRules/Movement/VerticalREL_PATH_SUFFIX), they aren't moved, and they take 1 damage for each square they would have been force moved. If the forced movement is [vertical](REL_PATH_PREFIXRules/Movement/VerticalREL_PATH_SUFFIX), the creature is moved through the dirt as if it were air.
...
- **≤11:** You fall off the creature into an unoccupied adjacent space of your choice, taking falling damage and landing [prone](REL_PATH_PREFIXRules/Conditions/ProneREL_PATH_SUFFIX) as usual (see [Falling](#page-283-3) below).
...
- **≤11:** You don't [jump](REL_PATH_PREFIXRules/Movement/JumpREL_PATH_SUFFIX) any farther than your baseline [jump](REL_PATH_PREFIXRules/Movement/JumpREL_PATH_SUFFIX) allows.
- **12-16:** You [jump](REL_PATH_PREFIXRules/Movement/JumpREL_PATH_SUFFIX) 1 square longer and higher than your baseline [jump](REL_PATH_PREFIXRules/Movement/JumpREL_PATH_SUFFIX) allows.
- **17+:** You [jump](REL_PATH_PREFIXRules/Movement/JumpREL_PATH_SUFFIX) 2 squares longer and higher than your baseline [jump](REL_PATH_PREFIXRules/Movement/JumpREL_PATH_SUFFIX) allows.

You can't [jump](REL_PATH_PREFIXRules/Movement/JumpREL_PATH_SUFFIX) farther or higher than the distance of the effect that allows you to move. You can't [jump](REL_PATH_PREFIXRules/Movement/JumpREL_PATH_SUFFIX) out of difficult terrain or damaging terrain (see below).

##### Crawl

If you are [prone](REL_PATH_PREFIXRules/Conditions/ProneREL_PATH_SUFFIX) (see [Conditions](#page-91-2) in [Chapter 5: Classes](#page-83-2)), you can remain [prone](REL_PATH_PREFIXRules/Conditions/ProneREL_PATH_SUFFIX) and [crawl](REL_PATH_PREFIXRules/Movement/CrawlREL_PATH_SUFFIX) on the ground. Doing so costs you 1 additional square of movement for every square you [crawl](REL_PATH_PREFIXRules/Movement/CrawlREL_PATH_SUFFIX). If you intentionally want to [crawl](REL_PATH_PREFIXRules/Movement/CrawlREL_PATH_SUFFIX), you can fall [prone](REL_PATH_PREFIXRules/Conditions/ProneREL_PATH_SUFFIX) as a free maneuver. While voluntarily [prone](REL_PATH_PREFIXRules/Conditions/ProneREL_PATH_SUFFIX), you can choose to stand as a free maneuver.
...
A creature who has "hover" in their speed entry (most commonly alongside "[fly](REL_PATH_PREFIXRules/Movement/FlyREL_PATH_SUFFIX)" or "[teleport](REL_PATH_PREFIXRules/Movement/TeleportREL_PATH_SUFFIX)"), or who gains the temporary ability to [hover](REL_PATH_PREFIXRules/Movement/HoverREL_PATH_SUFFIX), can remain motionless in midair. They don't fall even if they are knocked [prone](REL_PATH_PREFIXRules/Conditions/ProneREL_PATH_SUFFIX) or their speed is reduced to 0.
...
When a creature falls 2 or more squares and lands on the ground, they take 2 damage for each square they fall (to a maximum of 50 damage) and land [prone](REL_PATH_PREFIXRules/Conditions/ProneREL_PATH_SUFFIX). A creature who falls can reduce the effective height of the fall by a number of squares equal to their Agility score (to a minimum of 0). Falling into liquid that is 1 square or more deep reduces the effective height of a fall by 4 squares (to a minimum of 0).
...
When you force move a target, you can always move that target fewer squares than the number indicated. For example, when the [conduit](REL_PATH_PREFIXRules/Classes/ConduitREL_PATH_SUFFIX) obtains a tier 3 "push 3" outcome with their [Call the Thunder Down](REL_PATH_PREFIXRules/Abilities/Conduit/1st-Level%20Features/Call%20the%20Thunder%20DownREL_PATH_SUFFIX) ability, they can push targets any distance up to 3 squares, including choosing to not move certain targets at all.
...
If you can't [fly](REL_PATH_PREFIXRules/Movement/FlyREL_PATH_SUFFIX) and are force moved across an open space that would cause you to fall, such as being pushed over the edge of a cliff, you continue moving the total distance you were moved first. If you are still in a position to fall when the forced movement ends, you fall.

##### Stability

Each creature has a [stability](REL_PATH_PREFIXRules/Movement/StabilityREL_PATH_SUFFIX) that allows them to resist forced movement. When a creature is force moved, they can reduce that movement up to a number of squares equal to their [stability](REL_PATH_PREFIXRules/Movement/StabilityREL_PATH_SUFFIX). Heroes start with [stability](REL_PATH_PREFIXRules/Movement/StabilityREL_PATH_SUFFIX) 0 and can increase their [stability](REL_PATH_PREFIXRules/Movement/StabilityREL_PATH_SUFFIX) through ancestry, class, and kit options.

A creature's [stability](REL_PATH_PREFIXRules/Movement/StabilityREL_PATH_SUFFIX) can't be less than 0, even when reduced by a penalty.
...
A creature who uses the [Aid Attack](REL_PATH_PREFIXRules/Abilities/Common/Maneuvers/Aid%20AttackREL_PATH_SUFFIX) maneuver chooses an enemy adjacent to them. The next ability roll an ally makes against that enemy before the start of the aiding creature's next turn gains an edge.
...
A creature who is [grabbed](REL_PATH_PREFIXRules/Conditions/GrabbedREL_PATH_SUFFIX) by another creature, an object, or an effect (see [Grab](#page-287-5) below) can attempt to escape by using the following ability.
...
A creature can use this main action to make a [free strike](REL_PATH_PREFIXRules/Abilities/Common/Main%20Actions/Free%20StrikeREL_PATH_SUFFIX) (see [Free](#page-289-1) [Strikes](#page-289-1) below). Most of the time, you'll want to use the more impactful main actions granted by your class, kit, or other feature, just as the Director will use the main actions in a creature's stat block, but [free strikes](REL_PATH_PREFIXRules/Abilities/Common/Main%20Actions/Free%20StrikeREL_PATH_SUFFIX) are available for when all else fails. For instance, a [fury](REL_PATH_PREFIXRules/Classes/FuryREL_PATH_SUFFIX) who has no other options for ranged strikes might use the Ranged Weapon [Free Strike](REL_PATH_PREFIXRules/Abilities/Common/Main%20Actions/Free%20StrikeREL_PATH_SUFFIX) ability with an improvised weapon when battling a [flying](REL_PATH_PREFIXRules/Movement/FlyREL_PATH_SUFFIX) foe.
...
Whenever a creature has an enemy adjacent to them and the enemy willingly moves to a space that isn't adjacent to the creature without shifting, the creature can take advantage of that movement to quickly make a melee [free strike](REL_PATH_PREFIXRules/Abilities/Common/Main%20Actions/Free%20StrikeREL_PATH_SUFFIX) against the enemy as a free triggered action. This is called an opportunity attack.
...
Every hero has two standard [free strike](REL_PATH_PREFIXRules/Abilities/Common/Main%20Actions/Free%20StrikeREL_PATH_SUFFIX) abilities available to them. Your class might give you additional [free strike](REL_PATH_PREFIXRules/Abilities/Common/Main%20Actions/Free%20StrikeREL_PATH_SUFFIX) options, and your kit can improve the standard options (see [Chapter 6: Kits](#page-231-0)).

A melee weapon [free strike](REL_PATH_PREFIXRules/Abilities/Common/Main%20Actions/Free%20StrikeREL_PATH_SUFFIX) is a melee strike made with an unarmed strike or an improvised weapon. A ranged weapon [free strike](REL_PATH_PREFIXRules/Abilities/Common/Main%20Actions/Free%20StrikeREL_PATH_SUFFIX) is a ranged strike made with an improvised weapon. At the Director's discretion, the damage type of an improvised weapon can change based on the object used. For example, if you use a burning torch as an improvised weapon, it could deal fire damage when used for a [free strike](REL_PATH_PREFIXRules/Abilities/Common/Main%20Actions/Free%20StrikeREL_PATH_SUFFIX).
...
Your hero's survivability is represented by your Stamina. Think of Stamina as a combination of a creature's physical vitality and their overall energy for dodging and resisting incoming blows, spells, and other violence. It's not that every instance of damage deals a [bleeding](REL_PATH_PREFIXRules/Conditions/BleedingREL_PATH_SUFFIX) wound to you, but that each one chips away at your ability to fight effectively. One attack might make you sweat as you leap [back](REL_PATH_PREFIXRules/Abilities/Fury/1st-Level%20Features/BackREL_PATH_SUFFIX) to avoid an arrow, while another might graze your elbow with a dagger nick, leaving a dull, distracting pain. Eventually, this draining of energy leaves you open for bigger blows that can truly harm your body—or possibly kill you.
...
When your Stamina is 0 or lower, you are dying. While dying, you can't use the [Catch Breath](REL_PATH_PREFIXRules/Abilities/Common/Maneuvers/Catch%20BreathREL_PATH_SUFFIX) maneuver in combat. Additionally, you are [bleeding](REL_PATH_PREFIXRules/Conditions/BleedingREL_PATH_SUFFIX), and this instance of the condition can't be negated or removed in any way until you are no longer dying. While you are dying, you can still act, your allies can help you spend Recoveries in combat, and you can spend Recoveries out of combat as usual.

While your Stamina is lower than 0, if it reaches the negative of your winded value, you die. When you die, you can't be brought [back](REL_PATH_PREFIXRules/Abilities/Fury/1st-Level%20Features/BackREL_PATH_SUFFIX) to life without the use of a special powerful item, such as a Scroll of Resurrection.
...
While you are unconscious, you can't take main actions, maneuvers, triggered actions, free triggered actions, or free maneuvers; your speed is 0; you are unaware of your surroundings; and you are [prone](REL_PATH_PREFIXRules/Conditions/ProneREL_PATH_SUFFIX). Ability rolls against you have a double edge. If you wake up from being unconscious, you can stand up from [prone](REL_PATH_PREFIXRules/Conditions/ProneREL_PATH_SUFFIX) as a free maneuver.
...
The Director can decide that a well-made or poorly made object has more or less Stamina. Destroying a supernatural object often (but not always) requires a specific quest, such as throwing a magic ring [back](REL_PATH_PREFIXRules/Abilities/Fury/1st-Level%20Features/BackREL_PATH_SUFFIX) into the volcano where it was forged.
...
If your mount dies, they fall [prone](REL_PATH_PREFIXRules/Conditions/ProneREL_PATH_SUFFIX), and you fall off them and land [prone](REL_PATH_PREFIXRules/Conditions/ProneREL_PATH_SUFFIX) in the nearest unoccupied space of your choice.
...
Event endings can cover big scenarios such as the characters destroying a dam to unleash a river upon their enemies, or completing a ritual that causes all the demons they've been battling to be sent [back](REL_PATH_PREFIXRules/Abilities/Fury/1st-Level%20Features/BackREL_PATH_SUFFIX) to the Abyssal Wasteland, accompanied by visual details. For example, if the heroes are battling a necromancer who controls a horde of zombies, the undead might all crumble to dust when the necromancer is defeated. If the heroes destroy an eldritch machine sapping the land of its natural energy, the shockwave from the device's destruction could vaporize the cultists attempting to protect it.

<!-- -->
> **Fleeing Foes**
>
> If you've played a fantasy RPG before, odds are you've had an encounter where you didn't chase down every last fleeing foe—and then one such foe [grabbed](REL_PATH_PREFIXRules/Conditions/GrabbedREL_PATH_SUFFIX) another bunch of evil buddies and came [back](REL_PATH_PREFIXRules/Abilities/Fury/1st-Level%20Features/BackREL_PATH_SUFFIX) to ambush you. It takes only one experience like this to create players who promise, "No survivors. No mercy!" whenever foes break ranks. Chasing down every last foe can be fun once in a while, but it can easily turn a tactical encounter into a slog.
>
> Luckily, this is a heroic game. Although the Director can surprise the players with [dramatic reveals](REL_PATH_PREFIXRules/Abilities/Troubadour/8th-Level%20Features/Dramatic%20RevealREL_PATH_SUFFIX) and twisty-turny stories, "Gotcha!" moments that make players suspicious of every fleeing bandit shouldn't be part of those stories. If a bandit is fleeing an encounter, they're running away to rethink their life. If they're going for help, the players should get some sense of that—for example, the bandit screaming at the top of their lungs for help as they run toward their leader's tent. That way, the players can process what's happening, and will understand that stopping that fleeing bandit is part of the challenge of the encounter.
