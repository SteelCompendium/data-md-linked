...
**Grace (playing Val, a [conduit](REL_PATH_PREFIXRules/Classes/ConduitREL_PATH_SUFFIX)):** *Before anyone steps on them, I want to examine the symbols and figure out what they mean.*
...
**Director:** *The whole tower starts to sway as the sarcophagus lids crash to the floor and clawed undead hands emerge from within. Six decaying [devils](REL_PATH_PREFIXRules/Ancestries/DevilREL_PATH_SUFFIX), each tattooed with glowing green runes, rise. They're eager for violence.*
...
Each characteristic has a score that runs from −5 to +5. The higher a score, the more impact a creature has with that characteristic. A baby bunny rabbit would have a Might score of −5, while an ancient dragon would have a Might score of 5. The average [human](REL_PATH_PREFIXRules/Ancestries/HumanREL_PATH_SUFFIX) has a score of 0 in all their characteristics. Characteristic scores are added to power rolls—the dice rolls you make whenever your character attempts a task with an uncertain outcome (see [Power Rolls](#page-18-0) below).
...
Under certain circumstances, you might have one or more edges and banes on the same roll. For instance, you might take a bane when [weakened](REL_PATH_PREFIXRules/Conditions/WeakenedREL_PATH_SUFFIX) by poison, even as you gain an edge for striking a [prone](REL_PATH_PREFIXRules/Conditions/ProneREL_PATH_SUFFIX) creature. In general, edges and banes cancel each other out, resolving as follows:
...
When you spend a Recovery, you regain Stamina equal to your **recovery value**, which is one-third your Stamina maximum. You can spend your Recoveries with a special maneuver (see below), or you might do so with a little supernatural help from a [conduit](REL_PATH_PREFIXRules/Classes/ConduitREL_PATH_SUFFIX), a boost of adrenaline from an allied [tactician](REL_PATH_PREFIXRules/Classes/TacticianREL_PATH_SUFFIX), or inspiration from your party's [troubadour](REL_PATH_PREFIXRules/Classes/TroubadourREL_PATH_SUFFIX).

##### Spending Recoveries

During combat encounters and similarly dangerous situations when time is tracked in rounds (see [Combat](#page-279-4)), you can use the [Catch Breath](REL_PATH_PREFIXRules/Abilities/Common/Maneuvers/Catch%20BreathREL_PATH_SUFFIX) maneuver to regain Stamina. (See [Catch Breath](#page-287-2) in [Maneuvers](#page-287-0) for more information.) Some heroes have abilities that allow them or their allies to spend more Recoveries without using the [Catch Breath](REL_PATH_PREFIXRules/Abilities/Common/Maneuvers/Catch%20BreathREL_PATH_SUFFIX) maneuver.
...
Scattered across it all, Vasloria boasts the densest collection of elf-haunted wodes in Orden. All regions have wodes, but Vasloria's northern border is the Great Wode where the world still works as it did before [humans](REL_PATH_PREFIXRules/Ancestries/HumanREL_PATH_SUFFIX) arrived.
...
The last plane of law, the Seven Cities of Hell is among the lowest of the lower planes. A land of [devils](REL_PATH_PREFIXRules/Ancestries/DevilREL_PATH_SUFFIX) proud of their civilization, each of the seven cities is ruled by an archduke who schemes to ascend to the Throne of Hell. A world of bureaucratic law, the [devil](REL_PATH_PREFIXRules/Ancestries/DevilREL_PATH_SUFFIX) denizens of Hell have little interest in the other planes. Life is so much more *interesting* down here.
